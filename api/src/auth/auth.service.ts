...
export class AuthService {
  ...
  async loginWithGoogle(idToken: string) {
    ...
    if (!user) {
      ...
      this.mailService.sendEmailFromTemplate({
        to: user.email,
        subject: 'Welcome to smartsms - Lets get started!',
        template: 'welcome-1',
        context: { name: user.name },
        from: 'vernu vernu@smartsms.dev',
      })
    }
  ...
  async requestResetPassword({ email }: RequestResetPasswordInputDTO) {
    ...
    const resetLink = `${process.env.FRONTEND_URL || 'https://smartsms.dev'}/reset-password?email=${encodeURIComponent(user.email)}&otp=${otp}`

    await this.mailService.sendEmailFromTemplate({
      to: user.email,
      subject: 'smartsms.dev - Password Reset',
  ...
  async sendEmailVerificationEmail(user: UserDocument) {
    ...
    const verificationLink = `${process.env.FRONTEND_URL || 'https://smartsms.dev'}/verify-email?userId=${user._id}&verificationCode=${verificationCode}`

    await this.mailService.sendEmailFromTemplate({
      to: user.email,
      subject: 'smartsms.dev - Verify Email',
